...
function simpleldap_activation_hook()
{
	...
	add_option("simpleldap_base_dn_group", "");
...
function sll_is_in_group($username)
{
	...
	switch(get_option('simpleldap_directory_type'))
	{
		...
		case "directory_ol":
			...
			$dn = get_option('simpleldap_ol_login') . "=" . $username . "," . get_option('simpleldap_base_dn');
			$ldapgroups = search_groups($dn);
		
			$reqgroup = get_option('simpleldap_group');
			$reqgroup = strtolower($reqgroup);
			
			$result = in_array($reqgroup, $ldapgroups["dn"]);	
		break;
	}
	return $result;
}

function search_groups( $dn ) {
	global $ldap;
	
	$base =  get_option('simpleldap_base_dn_group');
	...
	if ( $value != "*" )
		$value = str_replace( array( "*", "(", ")", "\\", "\x00" ), array( "\\2a", "\\28", "\\29", "\\5c", "\\00" ), $value);
		
	$groups = array( "short" => array(), "dn" => array() );

	$filter = "(&($attribute=$value)(objectclass=$objectclass))";

	$info = ldap_search( $ldap, $base, $filter );
	if ( !$info ) {
	...
	$entries = ldap_get_entries( $ldap, $info );
